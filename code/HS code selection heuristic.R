...
phrases.finished=subset(job.phrase, processed==T & job.id %in% subset(job.log, nr.of.checks>1)$job.id)$phrase.id
...
estimation.set=subset(hs.candidates, selection.share>hs.out.threshold & selection.share<hs.in.threshold)

training.set=subset(hs.candidates, (selection.share<=hs.out.threshold)|(selection.share>=hs.in.threshold))
training.set$evaluation=as.numeric(training.set$selection.share>.5)
