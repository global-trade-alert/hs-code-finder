gta_hs_create_classifier_variables_phrase<- function(phrase.ids=NULL,
  ...
  hs.candidates=merge(hs.candidates, chosen.suggestions, by=c("phrase.id","hs.code.6"), all.x=T)
  ...
  for(u.id in app.users){
    ...
    if(any(user.checks %in% phrase.checks)){
      ...
      mx.rnd=aggregate(processing.round ~phrase.id, u.check, max)
      u.check=merge(u.check, mx.rnd, by=c("phrase.id","processing.round"))
      
      u.check=unique(u.check)
    ...
    hs.candidates[is.na(hs.candidates)]="Not checked"
