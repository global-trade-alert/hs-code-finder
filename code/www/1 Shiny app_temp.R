...
path="17 Shiny/5 HS code finder/database/GTA tariff database - 2.Rdata"
                    ...
                    tags$div(class = "settings-inner",
                             ...
                             tags$div(class = "search-field app-switcher-search",
                                      ...
                                      tags$div(class="search-tab tab-pane fade", id="search",
                                               ...
                                               tags$div(class="related tab-pane fade", id="related",
                                               ...
                                               tags$div(class="unrelated tab-pane fade", id="unrelated",
                                                        ...
                                                        tags$div(class="selection-unrelated-wrap",
                                                                 ...
                                                                 # label=NULL,
                                                                 # placeholder=""),
                                                                 actionButton("copy_unrelated_selection",
...
server <- function(input, output, session) {
  ...
  selected.rows <- function() {
    print(paste("Function selected rows 1: ", paste(data.ledger$hs.code.6[data.ledger$selected == 1], collapse = ", ")))
    isolate({
  ...
  observeEvent(input$users, {
    if(input$users != "Select") {
      ...
      phr.id <<- sample(job.phrase$phrase.id,1)
  ...
  eventReactive(input$query.refine, {
  ...
  observeEvent(input$copy_unrelated_selection, { 
    runjs("var copyText = document.getElementById('unrelated_selected_codes');
          copyText.select();
  ...
  observeEvent(input$not.product, {
    load(file=path)
    report.services.new <- data.frame(phrase.id = phr.id,
                                      user.id = users$user.id[users$name == input$users])
    report.services <<- rbind(report.services, report.services.new)
    rm(report.services.new)
    save_all()
    refresh_names()
  })
  ...
  observeEvent(input$create.user, {
    # load(file = "17 Shiny/5 HS code finder/database/GTA tariff database - 2.Rdata")
    name = input$username
    print(name)
    users <<- rbind(users, data.frame(user.id = max(users$user.id)+1,
                                      ...
                                      gta.layer = "core",
                                      email = "name@mail.com"))
    updateSelectInput(session, "users", choices = unique(users$name))
    reset("username")
    save_all()
  ...
  save_selection <- function(type) {
    ...
    if(input$users=="Select"){
      ...
      if (type %in% c("standard","clipboard")) {
        ...
        print(paste("OBSERVE SAVE SELECTION ALL BUTTON: "))
        
        load(file=path)        
        # check if phrase has been adjusted
        # Phrase.table
        if (! paste(input$query.refine, collapse=" ") %in% unique(phrase.table$phrase)) {
          phrase.table.new <- data.frame(phrase.id = max(phrase.table$phrase.id)+1,
                                         phrase = tolower(paste(input$query.refine, collapse = " ")),
                                         source = "adjusted")
          phrase.table <- rbind(phrase.table, phrase.table.new)
        ...
        # words.removed
        words.all <- paste(unlist(strsplit(as.character(phrase.table$phrase[phrase.table$phrase.id == phr.id])," ")))
        removed <- words.all[! words.all %in% paste(unlist(strsplit(as.character(input$query.refine)," ")))]
        ...
        nr.of.checks <- job.log$nr.of.checks[job.log$job.id == job.id]
        #get all phrases from check.phrases
        all.phrases <- job.phrase$phrase.id[job.phrase$phrase.id %in% subset(job.phrase, job.id == job.id)$phrase.id]
        #tally them up
